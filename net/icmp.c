...
/*debug purpose*/
kprintf("\nFile icmp.c; Function icmp_in; function: replypkt's datagram id: %d\n", replypkt->net_ipid);
...
kprintf("\nFile icmp.c; Function icmp_recv; msg before: %d\n",msg);
		msg = recvclr();
kprintf("\nFile icmp.c; Function icmp_recv; msg after recvclr: %d\n",msg);
/***/
timeout=30000;
		msg = recvtime(timeout);	/* Wait for a reply */
kprintf("\nFile icmp.c; Function icmp_recv; msg after recvtime: %d\n",msg);
		icmptr->icstate = ICMP_USED;
		if (msg == TIMEOUT) {
kprintf("\nTIMEOUT in icmprecv\n");
			restore(mask);
...
/*debug purpose*/
kprintf("\nFile icmp.c; Function icmp_recv; function: pkt's datagram id: %d\n", pkt->net_ipid);
...
/*debug purpose*/
kprintf("\nFile icmp.c; Function icmp_send; Sent pkt's datagram id: %d\n", pkt->net_ipid);
	restore(mask);
	return retval;
}
