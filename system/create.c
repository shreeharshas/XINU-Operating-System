...
//kprintf("Process name is %s\n",name);
        if (strncmp(name,"getmemforever",8) == 0){
//kprintf("Process name is %s\n",name);
	if (((saddr = (uint32 *)getstkforEver(ssize)) ==
	    (uint32 *)SYSERR ) ||
...
else{if (((saddr = (uint32 *)getstk(ssize)) ==
	    (uint32 *)SYSERR ) ||
	    (pid=newpid()) == SYSERR || priority < 1 ) {
		restore(mask);
		return SYSERR;
...
prptr->prhasmsg = FALSE;/*make it back to FALSE*/
